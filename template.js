...
if (!dirName) {
  ...
  console.log('示例：yarn run tpl test');
  process.exit(0);
}
...
const model: Model & ModelType = {
  ...
  //将接收到的put操作更新到model的state上
  //分发给所有connect这个model的页面
  reducers: {
    save(state, { payload }) {
...
// service页面模版
const serviceTep = `export const ${dirName} = async (data) => await Request({ url: '/url', method: 'GET', data });
`;
...
if (!appConfigFile.includes(`pages/${dirName}/index`)) {
  ...
  fs.writeFileSync('./src/app.config.ts', newAppConfigFile);
}
